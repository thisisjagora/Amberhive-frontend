...
const NewReleases = () => {
  ...
  const { newReleases, statusNewReleases, error } = useSelector(
    (state) => state.books
  );
  ...
  return (
    <div className="max-w-[85rem] mx-auto px-4 py-16">
      <div className="mb-8 flex justify-between items-center">
        <h2 className="text-xl font-bold text-gray-900">New Releases</h2>
        <Link
          to="/new-releases"
      ...
      {statusNewReleases === "succeeded" && newReleases.length === 0 && (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-6">
          {newReleases.slice(0, 5).map((book) => (
            <Link
              ...
              key={book.id}
            >
              <div className="group p-3 bg-white rounded-md shadow-sm hover:shadow-md transition">
                <div className="flex flex-col items-center">
