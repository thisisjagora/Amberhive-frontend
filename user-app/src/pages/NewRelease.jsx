...
import { fetchNewReleases } from "@/store/slice/bookSlice";
...
const NewRelease = () => {
  ...
  const { newReleases, statusNewReleases, error, paginationNewReleases } =
  ...
  const handlePageChange = (page) => {
    if (page >= 1 && page <= paginationNewReleases.last_page) {
  ...
  useEffect(() => {
    let filtered = newReleases.filter((book) => {
  ...
  }, [newReleases, sortOption, minPrice, maxPrice, selectedSubcategories]);
        ...
        <div className="hidden md:flex flex-col md:flex-row justify-between md:items-center gap-4 mb-6">
          {paginationNewReleases && (
            <p className="text-sm text-gray-700">
              Showing <strong>{paginationNewReleases.from}</strong> –{" "}
              <strong>{paginationNewReleases.to}</strong> of{" "}
              <strong>{paginationNewReleases.total}</strong> results
            </p>
          )}
          ...
          <div className="border border-gray-200 rounded-md p-2">
            ...
            <div className="flex justify-between items-center mt-6 p-2 text-gray-700 text-sm">
              <button
                ...
                disabled={!paginationNewReleases.prev_page_url}
                className={`cursor-pointer ${
                  !paginationNewReleases.prev_page_url
                    ? "text-gray-400"
                    : "hover:underline"
                }`}
              >
              ...
              <div className="flex gap-2">
                {Array.from(
                  { length: paginationNewReleases.last_page },
                  (_, i) => i + 1
                ).map((page) => (
                  <span
                    key={page}
              ...
              <button
                onClick={() => handlePageChange(currentPage + 1)}
                disabled={!paginationNewReleases.next_page_url}
                className={`cursor-pointer ${
                  !paginationNewReleases.next_page_url
                    ? "text-gray-400"
                    : "hover:underline"
                }`}
              >
