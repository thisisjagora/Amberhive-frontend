...
const EditPreviewStep = ({ id, bookData, prevStep, setStep }) => {
  const dispatch = useDispatch();
  const { genres } = useSelector((state) => state.genre);
  const navigate = useNavigate();
  const { statusUpdateBook } = useSelector((state) => state.books);
  ...
  const handleSave = async () => {
    try {
      ...
      // Always save with "pending" since draft is removed
      const dataToSave = { ...bookData, status: "pending" };
      ...
      console.log("Submitting formData:", dataToSave);

      // Dispatch update
      const response = await dispatch(updateBook({ id, formData })).unwrap();
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6 space-y-6">
          ...
          <img
            src={
              bookData.cover_image instanceof File
                ...
                : `https://test.amber-hive.com/storage/${bookData.cover_image}`
            }
            alt="Book Cover"
        ...
        <div className="space-y-4">
          {[
            ...
            {
              label: "Genre",
              value: genres.find((g) => g.id === bookData.genre_id)?.name,
            },
...
export default EditPreviewStep;
