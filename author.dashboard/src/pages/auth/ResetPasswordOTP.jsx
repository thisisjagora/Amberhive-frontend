...
import { resendOtp } from "@/redux/slices/authSlice"; // ✅ Corrected
import { verifyResetOtp } from "@/redux/slices/resetPasswordSlice";
...
const ResetPasswordOTP = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      await dispatch(verifyResetOtp({ otp })).unwrap();
  ...
  const handleResend = async () => {
    ...
    try {
      const resendResult = await dispatch(resendOtp({ email })).unwrap();

      // ✅ Trigger the queue only after successful OTP resend
      fetch("https://test.amber-hive.com/api/run-queue?token=secret123").catch(
        (err) => console.error("Queue trigger failed:", err)
      );
              ...
              <div className="flex justify-between items-center text-sm text-gray-600">
                ...
                <button
                  ...
                  className={`flex items-center gap-1 ${
                    timeLeft > 0 || loadingResend
                      ? "text-gray-400 cursor-not-allowed"
                      : "text-yellow-600 font-[600] hover:underline"
                  }`}
                  disabled={timeLeft > 0 || loadingResend}
                >
                  {loadingResend ? (
                    <>
                      <span className="animate-spin h-3 w-3 border-t-2 border-yellow-600 rounded-full" />
                      Resending...
                    </>
                  ) : (
                    "Resend"
