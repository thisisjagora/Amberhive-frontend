...
const ConfirmOTP = () => {
  ...
  const location = useLocation(); // Added to properly access location state
  const email = location.state?.email || "your email"; // Fallback to "your email" if not provided

  const { status, error } = useSelector((state) => state.auth);

  // Function to mask the email (e.g., "olive@gmail.com" becomes "oli****@gmail.com")
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await dispatch(verifyOtp(formData)).unwrap();
  ...
  const handleResend = async () => {
    try {
      await Promise.all([
        ...
        dispatch(resendOtp({ email })).unwrap(),
      ]);
      ...
      toast.error(error || "Could not resend OTP", {
        position: "top-right",
      });
    }
  };
                ...
                <button
                  ...
                  className={`hover:underline transition-colors duration-300 ${
                    timeLeft > 0
                      ...
                      : "text-yellow-600 font-[600] cursor-pointer"
                  }`}
                >
                  Resend
                </button>
