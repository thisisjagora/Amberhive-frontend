...
import { fetchOpenTickets } from "@/redux/slices/supportTicketSlice";
import { Badge } from "../ui/badge";
import OpenTicketCellAction from "./OpenTicketCellAction";
...
const OpenTicket = () => {
  ...
  const {
    openTickets,
    paginationOpenTickets: { currentPage, lastPage },
    statusFetchOpenTickets,
  } = useSelector((state) => state.supportTicket);

  useEffect(() => {
    dispatch(fetchOpenTickets(currentPage));
  }, [dispatch, currentPage]);

  const handlePageChange = (page) => {
    dispatch(fetchOpenTickets(page));
  };
  ...
  const openTicketcolumns = [
    ...
    },
    {
      ...
      header: "",
      cell: ({ row }) => {
        return <OpenTicketCellAction data={row.original} />;
      },
      size: 80,
    },
  ];
      ...
      <div className="bg-white border-[1px] rounded-lg w-full p-4 my-4 h-auto shadow-md">
        <div className="relative overflow-x-auto rounded-t-lg">
          <TableComponent
            ...
            showPagination={true}
            isLoading={statusFetchOpenTickets === "loading"}
            pagination={{
              currentPage,
              lastPage,
