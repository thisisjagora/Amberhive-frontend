...
const Sidebar = () => {
  const [openSections, setOpenSections] = useState(() => {
    const initialState = {};
    links.forEach((link) => {
      initialState[link.name.toLowerCase()] = false; // Initially all closed
      if (
        link.subLinks &&
        ...
        initialState[link.name.toLowerCase()] = true; // Open if a sub-link's path matches
      }
    });
    return initialState;
  });
            ...
            {links.map((link) => (
              <div key={link.name}>
                ...
                {link.subLinks.length > 0 ? (
                  ...
                  // If NO path (like Community), render normal DIV not clickable
